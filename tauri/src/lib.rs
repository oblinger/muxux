    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            if let Some(args) = overlay_args {
                eprintln!("[muxux] overlay args: pane={} x={} y={}", args.pane, args.x, args.y);
                if let Some(window) = app.get_webview_window("main") {
                    let overlay: tauri::State<OverlayState> = app.state();
                    overlay.show(args.pane);

                    // Center the 700px overlay on the given position
                    let half = OVERLAY_SIZE / 2;
                    let cx = args.x - half;
                    let cy = args.y - half;
                    eprintln!("[muxux] overlay position: raw=({},{}) centered=({},{})", args.x, args.y, cx, cy);
